...
| 2026-08-22 | Left Cloudflare Workflow RPC stub undisposed (`An RPC result was not disposed properly`)                    | In Cloudflare Workers RPC/Workflows, `WorkflowInstance` and RPC stub results implement `Disposable`. Always wrap workflow instance operations in `try/finally` and invoke `disposeRpc(instance)` to explicitly release RPC capabilities. |
