...
| :--- | :---: | :--- | :--- |
| **1. "The Hero Dish" AI Extraction & 3-Tier Fallback** | 🟡 **70% Done** | • Gemini 3.7 Flash multimodal vision extraction.<br>• `recommendedDishes`, `vibe`, `notes`, `cuisine` extracted.<br>• Drizzle schema stores dishes and vibes. | • Explicit `heroDish` / `vibeAnchor` prompt extraction.<br>• **Tier 2 Fallback:** Query Google Places `editorialSummary` / top review keywords for `communityFavoriteDish` when caption has no specific food item.<br>• **Tier 3 Fallback:** API support for user editable dish notes `[ ✍️ Add must-order ]`. |
| **2. "Food Crawl" Sequencing Engine (Course Logic)** | 🔴 **0% Done** | • Basic Lat/Lng coordinates resolved and stored in DB. | • Meal course classification (`courseType`: `'aperitif'` \| `'main'` \| `'digestif_dessert'` \| `'cafe_bakery'`).<br>• Backend sequencing engine (`POST /crawl/sequence`) ordering selected spots by dining timeline + walking time estimation. |
| **3. Direct Transactional Utility (Booking Deep-Links)** | 🟡 **40% Done** | • `websiteUrl`, `mapsUrl`, and `creatorNotes` resolved. | • Reservation provider detection (Resy, OpenTable, SevenRooms, Tock).<br>• Direct reservation URLs and actionable walk-in advice (e.g. *"Arrive 15 min before opening"*). |
| **4. "The Digital Tasting Menu" (Export & Clone)** | 🔴 **0% Done** | • Relational guides and crumbs tables defined in Drizzle. | • Dynamic editorial Tasting Menu card generation (Michelin-style graphic for IG Stories / iMessage).<br>• Public guide share endpoint (`GET /guides/share/:id`) + 1-Tap Clone endpoint (`POST /guides/clone/:id`). |
| **5. Database Persistence in Ingest Workflow** | 🟡 **60% Done** | • Drizzle schema & migrations generated (`posts`, `restaurants`, `post_restaurants`, `crumbs`, `guide_crumbs`). | • Connect `IngestWorkflow` Step 5 to Neon DB via Drizzle client.<br>• Fast-path deduplication cache check in `POST /ingest`. |
